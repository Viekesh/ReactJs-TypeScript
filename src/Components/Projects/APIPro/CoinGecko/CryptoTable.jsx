...
const CryptoTable = () => {
    ...
    return (
        <>
            <section className="crypto_options crypto_table crypto_block">
                {/* <button onClick={() => setTest("Modified")}>click</button> */}

                {/* {
                    cryptoData ? <table className="y_axis_center">

                        <thead>
                            <tr className="table_row y_axis_center">
                                <th>asset</th>
                                <th>name</th>
                                <th>price</th>
                                <th>total volume</th>
...

                        {
                            cryptoData.map((data) => {
                                return (
                                    <>
                                        <tbody>
                                            <tr key={data.id} className="tbody_table_row y_axis_center">
                                                <td className="y_axis_center">
                                                    <button>
                                                        <svg
                                                            className="star"
                                                            width="18"
                                                ...
                                                <td
                                                    className={
                                                        data.price_change_percentage_1h_in_currency > 0 ? "color: green" : "color:blue;"
                                                    }
                                                >{Number(data.price_change_percentage_1h_in_currency).toFixed(3)}</td>
                                                <td>{Number(data.price_change_percentage_24h_in_currency).toFixed(3)}</td>
                                                <td>{Number(data.price_change_percentage_7d_in_currency).toFixed(3)}</td>
                                            </tr>
                                        </tbody>
                                    </>
                                )
...

                {
                    cryptoData
                        ...
                        cryptoData.map((data) => {
                            return (
                                <>
                                    < div className="crypto_info" key={data.id}>
                                        <div className="crypto_name_img y_axis_center">
                                            <div className="crypto_img">
                                                <img src={data.image} alt={data.name} />
                                            </div>
                                            <h3 className="crypto_name">{data.name}</h3>
                                        </div>
                                        ...
                                        <div className="num_info">
                                            ...
                                            <h3>total volume : <span>{data.total_volume}</span></h3>
                                            <h3>mkt cap change : <span>{data.market_cap_change_24h}%</span></h3>
                                            {/* <h3
                                                className={
                                                    data.price_change_percentage_1h_in_currency > 0 ? "color: green" : "color:blue;"
                                                }
                                            >1h: {Number(data.price_change_percentage_1h_in_currency).toFixed(3)}</h3>
                                            <h3>24h: {Number(data.price_change_percentage_24h_in_currency).toFixed(3)}</h3>
                                            <h3>7 day: {Number(data.price_change_percentage_7d_in_currency).toFixed(3)}</h3> */}
                                        </div>
                                    </div>
                                </>
                            )
